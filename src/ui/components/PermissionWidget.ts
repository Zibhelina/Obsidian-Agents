...
export class PermissionWidget extends Component {
  ...
  constructor(
    ...
    super();
    this.containerEl = container.createDiv({ cls: "agentchat-permission-widget" });
    this.render(toolCall, onDecide);
  }

  private render(toolCall: ToolCall, onDecide: (decision: PermissionDecision) => void): void {
    const title = this.containerEl.createDiv({ cls: "agentchat-permission-title" });
    title.setText(`\u2699\ufe0f ${toolCall.name}`);

    const args = this.containerEl.createDiv({ cls: "agentchat-permission-args" });
    args.setText(JSON.stringify(toolCall.arguments, null, 2));

    const buttons = this.containerEl.createDiv({ cls: "agentchat-permission-actions" });

    const acceptBtn = buttons.createEl("button", {
      cls: "agentchat-btn-primary",
      text: "Accept",
    });
    const denyBtn = buttons.createEl("button", {
      cls: "agentchat-btn-danger",
      text: "Deny",
    });
    ...
    this.registerDomEvent(explainBtn, "click", () => {
      if (!explainArea) {
        explainArea = this.containerEl.createEl("textarea", {
          cls: "agentchat-permission-explain-area",
        });
